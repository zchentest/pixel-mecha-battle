...
使用 PeerJS 技术实现 P2P 直连，无需服务器中转，两人即可异地对战！

### 联机步骤

```
┌─────────────────────────────────────────────────────┐
│  ① 房主：输入昵称 → 点击「创建房间」→ 复制房间号    │
│  ② 好友：输入昵称 → 输入房间号 → 点击「加入房间」   │
│  ③ 连接成功后自动开始游戏！                         │
└─────────────────────────────────────────────────────┘
```
...
- **P2P直连**：使用 WebRTC 技术，两玩家直接建立连接
- **无需服务器**：PeerJS 提供免费信令服务，数据不经过第三方
...
- 两人需要在有网络的环境下使用
- 房间号有效期为连接建立前，超时请重新创建
- 建议关闭防火墙或路由器 NAT 穿透限制
